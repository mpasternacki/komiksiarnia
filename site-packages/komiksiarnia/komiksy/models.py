...
class Seria(models.Model):
    ...
    def tags(self):
        return Tag.get_tag_counts(seria=self)
...
class Pasek(models.Model):
    ...
    def tags(self):
        return Tag.get_tag_counts(pasek=self)
...
class Attachment(models.Model):
    ...
    def get_absolute_url(self):
        return 'http://komiksiarnia.net/strip/%d,%d' % ( self.pasek.id, self.sortorder )
    class Meta: 
        # managed = False
...
class Tag(models.Model):
    pasek = models.ForeignKey(Pasek, db_column='pasek')
    tag = models.CharField(max_length=90, blank=True)
    autor = models.ForeignKey(User, db_column='autor')
    seria = models.ForeignKey(Seria, db_column='seria')
    class Meta:
        db_table = u'tagi'

    @staticmethod
    def get_tag_counts(**criteria):
        rv = defaultdict(int)
        for tag in Tag.objects.filter(**criteria):
            rv[tag.tag] += 1
        return rv.items()
