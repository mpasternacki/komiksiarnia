...
from models import Seria, Pasek, Tag, User
...
def tag_list(request, seria):
    if seria == '*':
        ...
        tt = Tag.get_tag_counts()
    else:
        s = get_object_or_404(Seria, tytul=seria)
        tt = Tag.get_tag_counts(seria=s)
...
def tag(request, seria, tag):
    if seria == '*':
        ...
        t = 'paski wszystkich serii oznaczone tagiem ' + tag
    else:
        s = get_object_or_404(Seria, tytul=seria)
        tags = Tag.objects.filter(seria=s, tag=tag)
        t = 'paski z serii ' + s.pelny_tytul + ' oznaczone tagiem ' + tag
    def __paski(tags):
        for t in tags:
            try: yield t.pasek
            except Pasek.DoesNotExist: pass
    return render_to_response('komiksy/pasek_list.html',
                              dict(object_list=__paski(tags), title=t),
                              context_instance=RequestContext(request))
...
def pasek(request, seria, namiar):
    ...
    return render_to_response('komiksy/pasek_detail.html',
